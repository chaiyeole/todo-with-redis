...
import (
	...
	todoListRepo "github.com/chaiyeole/todo/repository"
...
func New(addr string, password string, db int) (todoListRepo.IFileRepo, error) {
...
// confused here with :map is always passed by pointer && new returns a pointer
...
func (r *repo) Set(ctx context.Context, id string, task models.Task) *domain.CustomError {
	...
	redisResponse := r.rdb.Set(context.Background(), id, jsonTask, 0)
...
func (r *repo) Get(ctx context.Context, id string) (*models.Task, *domain.CustomError) {
	redisResponse := r.rdb.Get(context.Background(), id)
